import { ChunkGenerator3 } from "@hello-worlds/planets"
...
import {
  ThreadParams,
  heightGenerator,
} from "../examples/moon/Moonworker.generators"
...
export function useGetExactPlanetaryElevation() {
  ...
  const [elevationFunc, setElevationFunction] = useState<ChunkGenerator3<
    ThreadParams,
    number,
    {}
  > | null>(null)
  useEffect(() => {
    async function loadMe() {
      return await heightGenerator(planet)
    }
    loadMe().then(val => {
      setElevationFunction(_ => {
        return val
      })
    })
  }, [planet])

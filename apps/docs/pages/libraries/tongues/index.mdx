...
const morpheme = conlang.makeMorheme()
...
- load word adaption from another `Language`
- deterministically simulate `Language` change
- generate a `Language` as a simulated offshoot branch from changes to a target `Language`'s phonology

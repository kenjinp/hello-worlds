...
const SyncedPosition: React.FC<{ connectionId: number }> = ({ connectionId }) => {
  const [position] = React.useState(tempVector3.set(0,0,0).clone());

  const [lastUpdateMeta] = React.useState<Explorer["lastUpdateMeta"]>({ positions: [] })
  ...
  useFrame(({ clock }) => {
    others.map(user => {
      ...
      const explorer = entities.find((entity) => entity.connectionId == connectionId);
      if (explorer) {
        ...
        //   }
        // )
        position.set(user.presence.position.x, user.presence.position.y, user.presence.position.z)
      }
    })
  })
...
export const ExplorerEntity: React.FC = () => {
  ...
  return (<>{
    others.map(user => {
      return user.presence ? (<ECS.Entity key={user.connectionId}>

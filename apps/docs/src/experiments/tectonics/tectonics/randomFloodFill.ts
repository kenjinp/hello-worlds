...
export function randomFloodFill(
  ...
  const assignRegionToPlate = (
    tectonics: Tectonics,
    region: Region,
    plate: Plate,
  ) => {
    assignedRegions[region.properties.index] = plate.index
    const plateRegion = new PlateRegion(region, plate)
    plate.regions.set(region.properties.index, plateRegion)
    // tectonics.cellSpacePartition.directlyAddEntity(
    //   plateRegion,
  ...
  while (fronts.reduce((memo, q) => memo || !q.isEmpty, false)) {
    ...
    for (let i = 0; i < fronts.length; i++) {
      ...
      if (!regionIsAlreadyAssigned(region)) {
        assignRegionToPlate(tectonics, region, plate)
